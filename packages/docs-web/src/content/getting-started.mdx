...
import { CardImage } from '@/app/_components/card-image';
...
<CardImage
  title="Dashboard"
  href="https://dashboard.productmint.io"
  src="/assets/dashboard_home.png"
  alt="ProductMint Dashboard"
/>
...
The steps below use the `@product-mint/ethers-sdk` package. These are all direct interactions with the smart contracts powering the ProductMint system. Both the dashboard and the CLI tool use this package under the hood.
...
<Callout type="error">
    Never store your private key in your code. Always use environment variables and consider using an [Organization Admin](/system-architecture/organization-admin) wallet to help reduce the risk of your organization being compromised.
</Callout>

### Mint Organization NFT

Mint the Organization NFT to represent your organization. Get the Organization NFT contract address from [contract addresses](/contract-addresses) for the network you are using.
...
Interested in using a dynamic token to have stable pricing in a volatile token? Learn more about them [here](/features/dynamic-tokens).
...
You are now ready to start selling your products and subscriptions by minting Product Pass NFTs. You can mint them directly from the contract on your website by calling the `purchaseProducts` function on the `PurchaseManager` contract.
...
**Using a React based framework?** Consider using [WAGMI](https://wagmi.sh) to write to the ProductMint contracts and providing the ABI via the `@product-mint/ethers-sdk` package.
...
While it is significantly easier to use ProductMint than a traditional payment processor API, it is still a bit of work. If you are looking for an easier way to configure your products and pricing models, check out the [dashboard](https://dashboard.productmint.io).